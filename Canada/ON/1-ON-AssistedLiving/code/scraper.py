...
def parse_directory(output_name='output', output_type='csv', region_name=None):
    ...
    head_row = ['Licence #', 'First Issue Date', 'Licensee Name', 'Address Line 1', 'Address Line 2', 'city', 'Postal Code', 'Province', 'Contact Phone', 'Fax', 'Primary Contact', 'Website', 'Email', 'Services Available', 'Resident Capacity', 'Automatic Sprinklers in All Suites?', 'Number of Suites with Automatic Sprinklers', 'Sprinkler Locations']
...
def download_details(file_name=None):
    ...
    for dir in dir_list:
        ...
        rows_src = tree.xpath('//tr')
        
        i=0
        for row in rows_src:
            facility_id = row.get('id')
            facility_url = 'http://rhra.ca/en/register/homeid/' + facility_id
            download(facility_url, 'details/' + facility_id + '.html', percent = "{:.0%} ".format(i/len(rows_src)))
            i+=1
