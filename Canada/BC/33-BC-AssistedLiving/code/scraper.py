...
def parse_directory(output_name='output', output_type='csv', region_name=None):
    ...
    head_row = ['Address Line1', 'email', 'Fax', 'Contact Phone', 'Postal Code', 'Province', 'Facility Name']
...
def parse_detail(file_name):
    ...
    data_src = tree.xpath('//td/text()')
    if len(data_src) == 0:
        print(empty)
        return None
    for i in range(0, len(data_src)):
        if re.search('<div', data_src[i]) is None:
            data.append(data_src[i])

    if re.search('@', data[1]) is None:
        data.insert(1, '')
    if re.findall('([A-Za-z]\d[A-Za-z] ?\d[A-Za-z]\d)|(\d{5})', data[3]):
        data.insert(3, '')
        data[3] = data[2]
        data[2] = ''
    del data[6]
