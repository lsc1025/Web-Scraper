...
def parse_directory(output_name='output', output_type='csv', region_name=None):
    ...
    dir_list = [item for item in os.listdir('../input/details') if re.search('.htm', item) is not None] 
    ...
    head_row = ['Facility Name', 'Area', 'Operator', 'Address', 'City', 'Postal Code', 'Province', 'Capacity']
...
def parse_detail(file_name):
    ...
    csvrow.append(tree.xpath('//h1/span/text()')[0].replace('â€™', "'")) # facility name

    basic_src = tree.xpath('//*[@id="HeaderContent"]//p[1]//text()')


    i = 0
    for data in basic_src:
        if (data.strip() == ''):
            del basic_src[i]
            continue
        basic_src[i] = data.strip()
        i+=1

    csvrow.append(basic_src[1])
    csvrow.append(basic_src[3])
    csvrow.append(basic_src[5])
    csvrow.append(basic_src[1])
    csvrow.append(basic_src[1])
    csvrow.append(basic_src[1])
    csvrow.append(basic_src[1])
...
def download_details(file_name='index.html'):
    ...
    for dir in dir_list:
        ...
        urls = tree.xpath('//*[@id="sideBar"]/div[1]//a')

        for url in urls:
            facility_url = url.get('href')
            if facility_url == None:
                continue
            facility_url = 'http://www.viha.ca' + facility_url
            facility_name = facility_url.split('/')[6]

            download(facility_url, '../input/details/' + facility_name , percent = "{:.0%} ".format(i/len(urls)))
            i+=1
