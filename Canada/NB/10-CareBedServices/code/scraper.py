# -*- coding: utf-8 -*-
# scraper.py for 10-NB-SpecialCareBeds
...
def parse_directory(output_name='output', region_name=None):
    ...
    csv_f = open("../output/" + output_name + '.csv', "w+")
    csv_writer = csv.writer(csv_f, lineterminator='\n')

    head_row = ['Facility Name', 'Region', 'Address', 'City', 'Province', 'Postal Code', 'Contact', 'Phone', 'Language', 'Available Space']
            ...
            for entry in details:
                ...
                if i == 2:
                    for address_entry in entry.split(','):
                        address_entry = address_entry.replace('\xa0', ' ').strip()
    ...
    csv_f.close()
    print('Output file saved: ' + output_name + '.csv')
    ...
    for dir in dir_list:
        ...
        for row in rows:
            if row[1].text is not None: # table head
                continue
            facility_name = row[1].xpath('./a')[0].text
            facility_name = facility_name.replace('"', '') # escape for ntfs file name requirements
            facility_name = facility_name.replace('/', '')
            url = "http://www1.gnb.ca/0017/careBed/IFrame/" + row[1].xpath('./a')[0].get('href')
