...
def parse_directory(output_name='output', output_type='csv', file_name=None):
    ...
    head_row = ['Facility Name', 'Address1', 'Address2', 'City', 'Postal Code', 'Province', 'Contact Phone', 'Link to Details']
...
def parse_detail(file_name):
    ...
    contact = tree.xpath('//div[@class="Normal"]/p[3]/text()')
    csvrow.append(contact[0].split(':')[1].strip())
    for i in range(0, 4):
        csvrow.append('')
    csvrow.append(contact[1].split(':')[1].strip())
    csvrow.append('http://paphr.ca/locations/care-homes/' + tree.xpath('//h1/text()')[0].replace(' ', '-').lower())
...
def download_details(file_name='index.html'):
    ...
    for dir in dir_list:
        ...
        urls = tree.xpath('//*[@id="dnn_ctr477_HtmlModule_lblContent"]/div/table/tbody/tr/th/a')

        for url in urls:
            facility_url = 'http://paphr.ca' + url.get('href')
            facility_name = facility_url.split('/')[5]
