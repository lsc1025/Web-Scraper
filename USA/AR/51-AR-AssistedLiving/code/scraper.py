...
def parse_directory(output_name='output', output_type='csv', file_name='51-AR.csv'):
    ...
    head_row = ['Facility Name', 'Map Indicator', 'Respite Provider Name', 'Address1', 'Contact Phone', 'City', 'Zip', 'State', 'Country']
    ...
    for dir in dir_list:
        ...
        for row in csv_input[1:]:
            address = parse_address(row[4])
            row[4] = row[5]
            del row[5]
            row += list(address.values())
            row.append('US')
...
def parse_address(source):
    ...
    postalcode = postalcode_re.findall(address_src[1])
    if postalcode:
        if (len(postalcode) > 1):
    ...
    address['province'] = province_re.findall(address_src[1])[0]
