...
def parse_directory(output_name='output', output_type='csv', file_name='index.html'):
    ...
    head_row = ['Facility Name', 'Address1', 'Address2', 'City', 'Zip', 'State', 'Administrator', 'Phone', 'Fax', 'License Number', 'License Exp.', 'Country']
    csv_writer.writerow(head_row)

    for dir in dir_list:
        print(dir)
        tree = get_lxml_tree('../input/' + dir)
        text_src = tree.xpath('//pre/text()')

        src = (parse_list(text_src[1].split('\r\n')))

        i = 2
        while i + 8 < len(src):
            csvrow = []
            csvrow.append(src[i])
            address = parse_address(src[i+1] + ',' + src[i+2])
            if re.search('Administrator', src[i+3]) is None: # no admin
                src.insert(i+3, 'Admin:')
            csvrow += list(address.values())
            for k in range(i+3, i+8):
                csvrow.append(src[k].split(':')[1].strip())
            csvrow.append('US')
            i += 8
...
def parse_detail(file_name):
    ...
    src = tree.xpath('//div[@class="inner-items"]/ul/li/div[2]/text()')

    address = parse_address(src[0])
    csvrow += list(address.values())
    contact = (tree.xpath('//div[@class="inner-items"]/ul/li/div[2]/a/text()'))


    if len(contact) < 2:
        contact.append('')
    if re.search('@', contact[1]) and len(contact) < 3:
        contact.append('')
    else:
        if len(contact) < 3:
            contact.insert(1, '')
