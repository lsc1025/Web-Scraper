...
def parse_directory(output_name='output', output_type='csv', file_name='index.html'):
    ...
    head_row = ['Facility Name', 'Address1', 'City', 'Zip', 'State', 'Phone', 'State Licensed Beds', 'Country']
    ...
    for dir in dir_list:
        ...
        src = tree.xpath('//li[@class="dblspace"]')

        for item in src:
    
            data = (parse_list(item.xpath('./text()')))

            address = parse_address(data[2].replace('\xa0\xa0', ' '))

            data.insert(2, address['city'])
            data.insert(3, address['postalcode'])
            data.insert(4, address['province'])

            del data[5]
            
            data[6] = data[6].split(':')[1].strip()
            data.append('US')
