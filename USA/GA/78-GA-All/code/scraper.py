...
def parse_directory(output_name='output', output_type='csv', file_name=None):
    ...
    head_row = ['Facility Name', 'Address1', 'Address2', 'City', 'Zip', 'State', 'Phone', 'Fax', 'Mailing Address1', 'Mailing Address2', 'Mailing City', 'Mailing Zip', 'Mailing State', 'Licensed Beds', 'Country']
...
def parse_detail(file_name):
    ...
    csvrow.append(tree.xpath('/html/body/center[2]/font/b/text()')[0])

    st_addr_src = parse_list(tree.xpath('//tr[2]/td/font/text()'))
    mal_addr = parse_address(parse_list(tree.xpath('//tr[4]/td/font/text()')))
    capacity = tree.xpath('//tr[9]/td/font/text()')[0].strip()
    st_addr = parse_address(st_addr_src[1:3])

    csvrow += list(st_addr.values())
    csvrow.append(st_addr_src[3].split(':')[1][:-3].strip())
    csvrow.append(st_addr_src[3].split(':')[2])
    csvrow += list(mal_addr.values())
    csvrow.append(capacity)
    csvrow.append('US')
...
def download_details(file_name=None):
    ...
    for dir in dir_list:
        ...
        for item in src:
            ...
            if re.search('home.asp', facility_url):
                continue
...
def parse_address(source):
    ...
    address['line2'] = '' if len(address_src) < 2 else address_src[1].strip()
